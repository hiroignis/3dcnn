...
def inference(box_placeholder, keep_prob, lstm_memory_size):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        with tf.name_scope('weights'):
            weight_filter = _weight_variable([4, 4, in_channels, out_channels])
    ...
    with tf.variable_scope('pool1') as scope:
        pool1 = tf.nn.max_pool2d(input=input_layer,
                                 ksize=[1, 3, 3, 1],
                                 strides=[1, 2, 2, 1],
                                 padding='VALID')
        norm1 = pool1
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        with tf.name_scope('weights'):
            weight_filter = _weight_variable([3, 3, in_channels, out_channels])
            variable_summaries(weight_filter)
    ...
    with tf.variable_scope('pool2') as scope:
        pool2 = tf.nn.max_pool2d(input=input_layer,
                                 ksize=[1, 3, 3, 1],
                                 strides=[1, 2, 2, 1],
                                 padding='VALID')
        norm2 = pool2
    ...
    with tf.variable_scope('fc5') as scope:
        ...
        fc5 = tf.nn.relu(features=units)
        # set dropout
        fc5_dropout = tf.nn.dropout(fc5, keep_prob)

    input_layer = fc5_dropout

    with tf.variable_scope('lstm6') as scope:
        input_layer_split = tf.split(input_layer, parameter.DATA_SEQUENCE_LENGTH, 1)

        cell = tf.contrib.rnn.BasicLSTMCell(FC_LAYER_SIZE, forget_bias=0.8, state_is_tuple=False)
        lstm6, _ = tf.contrib.rnn.static_rnn(cell,
                                             input_layer_split,
