...
from network import parameter as parameter
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter(parameter.SUMMARIES_PATH + parameter.RETRAIN_PATH + 'train/', sess.graph)
        test_writer = tf.summary.FileWriter(parameter.SUMMARIES_PATH + parameter.RETRAIN_PATH + 'test/', sess.graph)

        beginning_time = time.time()

        for step in range(parameter.MAX_STEPS):
            if step % parameter.SAVE_INTERVAL == 0 and step != 0:
                save_path = saver.save(sess, '/home/' + parameter.USER + '/type/tour/path/3dcnn/models/cnn/'
                                       + parameter.RETRAIN_PATH + parameter.RESTORE_MODEL, global_step=step)
        ...
        save_path = saver.save(sess, '/home/' + parameter.USER + '/type/tour/path/3dcnn/models/cnn/'
                               + parameter.RETRAIN_PATH + parameter.RESTORE_MODEL)
